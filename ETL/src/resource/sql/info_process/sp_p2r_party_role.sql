DELIMITER $$
USE info_process;
DROP PROCEDURE IF EXISTS info_process.sp_p2r_party_role;
CREATE PROCEDURE info_process.sp_p2r_party_role(IN p_date DATE)
BEGIN
...
DECLARE execution_time      INT;
	...
	SET process_name = "sp_p2r_party_role";
	
	
	SELECT CURRENT_TIMESTAMP INTO start_dtm ;
	SET step_number = "01";
	SET step_desc = "Create temporary table for party role from production data.";
	
TRUNCATE TABLE 	info_report.party_role;
...
INSERT INTO info_report.party_role
(bank_identifier, reporting_reference_date, party_identifier, role_of_party)
SELECT * 
FROM party_role_prod_tmp
;
